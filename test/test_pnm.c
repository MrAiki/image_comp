...
void testPNM_CreateFreePictureTest(void *obj)
{
  TEST_UNUSED_PARAMETER(obj);
  ...
  /* 割り当てテスト */
  {
    ...
    pnm = PNM_CreatePicture(20, 10);
    Test_AssertCondition(pnm != NULL);
    Test_AssertEqual(pnm->header.width, 20);
    Test_AssertEqual(pnm->header.height, 10);
    Test_AssertCondition(pnm->picture != NULL);

    for (y = 0; y < pnm->header.height; y++) {
      Test_AssertCondition(pnm->picture[y] != NULL);
    }

    PNM_DestroyPicture(pnm);
  }

  /* 割り当て失敗テスト */
  {
    Test_AssertCondition(PNM_CreatePicture(0, 1) == NULL);
    Test_AssertCondition(PNM_CreatePicture(1, 0) == NULL);
    Test_AssertCondition(PNM_CreatePicture(0, 0) == NULL);
  }
...
void testPNMParser_ReadHeaderTest(void *obj)
{
...

  {
    ...
    for (i_test = 0; 
      ...
      Test_AssertEqual(header.format, format_ans[i_test]);
...
#define TESTHEIGHT  6
    ...
    fp = fopen(test_filename, "rb");
    PNMParser_InitializeForText(&parser, fp);
    image = PNM_CreatePicture(TESTWIDTH, TESTHEIGHT);
    ...
    for (y = 0; y < TESTHEIGHT; y++) {
      for (x = 0; x < TESTWIDTH; x++) {
        Test_AssertEqual(PNMPict_BIT(image,x,y), answer[x][y]);
      }
    }

    /* 解放 */
    PNM_DestroyPicture(image);
    fclose(fp);
#undef TESTHEIGHT
#undef TESTWIDTH
  }
...
#define TESTHEIGHT  6
    ...
    for (y = 0; y < TESTHEIGHT; y++) {
      for (x = 0; x < TESTWIDTH; x++) {
        Test_AssertEqual(PNMPict_GRAY(image,x,y), answer[x][y]);
      }
    }
...
#define TESTHEIGHT  5
    ...
    for (y = 0; y < TESTHEIGHT; y++) {
      for (x = 0; x < TESTWIDTH; x++) {
        Test_AssertEqual(PNMPict_R(image,x,y), answer[x][y][0]);
        Test_AssertEqual(PNMPict_G(image,x,y), answer[x][y][1]);
        Test_AssertEqual(PNMPict_B(image,x,y), answer[x][y][2]);
      }
    }
    ...
    for (y = 0; y < TESTHEIGHT; y++) {
      for (x = 0; x < TESTWIDTH_STRIDE; x++) {
        for (shift = 0; shift < 8; shift++) {
          ...
          Test_AssertEqual(PNMPict_BIT(image,8*x+shift,y), ans);
        }
      }
    }
...
/* 実画像の読み込みテスト */
void testPNMParser_ReadPNMPictureTest(void *obj)
{
  TEST_UNUSED_PARAMETER(obj);
  ...
  /* 読み込みテスト */
  {
    ...
    for (i_test = 0; i_test < NUM_TESTS; i_test++) {
      /* 読み込み */
      image = PNM_CreatePictureFromFile(filename[i_test]);
      Test_AssertCondition(image != NULL);

      /* 正しく取得できたか確認 */
      Test_AssertEqual(image->header.format, format_answer[i_test]);
      Test_AssertEqual(image->header.width,  TESTIMG_WIDTH);
      Test_AssertEqual(image->header.height, TESTIMG_HEIGHT);
      ...
      if (format_answer[i_test] == PNM_P1
        ...
        for (y = 0; y < image->header.height; y++) {
          for (x = 0; x < image->header.width; x++) {
            ...
            if (PNMPict_BIT(image, x, y) != imgdata_answers[i_test][imginx]) {
              is_ok = 0;
              goto CHECK_END;
            }
          }
        ...
        for (y = 0; y < image->header.height; y++) {
          for (x = 0; x < image->header.width; x++) {
            imginx = y * image->header.width + x;
            if (PNMPict_GRAY(image, x, y) != imgdata_answers[i_test][imginx]) {
              is_ok = 0;
              goto CHECK_END;
            }
          }
        ...
        for (y = 0; y < image->header.height; y++) {
          for (x = 0; x < image->header.width; x++) {
            ...
            if (PNMPict_R(image, x, y) != imgdata_answers[i_test][imginx+0]
                || PNMPict_G(image, x, y) != imgdata_answers[i_test][imginx+1]
                || PNMPict_B(image, x, y) != imgdata_answers[i_test][imginx+2]) {
              is_ok = 0;
              goto CHECK_END;
            }
          }
...
/* 画像書き込みテスト */
void testPNMBitWriter_WritePNMPictureTest(void *obj)
{
  TEST_UNUSED_PARAMETER(obj);
...
#define TESTHEIGHT  17
    ...
    for (i_test = 0; i_test < sizeof(test_filename) / sizeof(test_filename[0]); i_test++) {
      /* 画像を作ってみる */
      image = PNM_CreatePicture(TESTWIDTH, TESTHEIGHT);
      image->header.format = formats[i_test];
      srand(0);
      for (y = 0; y < TESTHEIGHT; y++) {
        for (x = 0; x < TESTWIDTH; x++) {
          answer[x][y] = rand() % 2;
          PNMPict_BIT(image, x, y) = answer[x][y];
        }
      }

      /* 書き込み */
      Test_AssertEqual(PNM_WritePictureToFile(test_filename[i_test], image), PNM_APIRESULT_OK);
      PNM_DestroyPicture(image);

      /* ちゃんと作れているか確認 */
      image = PNM_CreatePictureFromFile(test_filename[i_test]);
      Test_AssertCondition(image != NULL);
      Test_AssertEqual(image->header.format, formats[i_test]);

      /* 内容確認 */
      is_ok = 1;
      for (y = 0; y < TESTHEIGHT; y++) {
        for (x = 0; x < TESTWIDTH; x++) {
          if (PNMPict_BIT(image, x, y) != answer[x][y]) {
            is_ok = 0;
            goto CHECK_END_P1P4;
          }
        }
...
#define TESTHEIGHT  50
    ...
    for (i_test = 0; i_test < sizeof(test_filename) / sizeof(test_filename[0]); i_test++) {
      ...
      image = PNM_CreatePicture(TESTWIDTH, TESTHEIGHT);
      image->header.format = formats[i_test];
      image->header.max_brightness = 0xFF;
      srand(0);
      for (y = 0; y < TESTHEIGHT; y++) {
        for (x = 0; x < TESTWIDTH; x++) {
          answer[x][y] = rand() % 0x100;
          PNMPict_GRAY(image, x, y) = answer[x][y];
        }
      }
      ...
      for (y = 0; y < TESTHEIGHT; y++) {
        for (x = 0; x < TESTWIDTH; x++) {
          if (PNMPict_GRAY(image, x, y) != answer[x][y]) {
            is_ok = 0;
            goto CHECK_END_P2P5;
          }
        }
    ...
    for (i_test = 0; i_test < sizeof(test_filename) / sizeof(test_filename[0]); i_test++) {
      ...
      for (y = 0; y < TESTHEIGHT; y++) {
        for (x = 0; x < TESTWIDTH; x++) {
          ...
          PNMPict_R(image, x, y) = answer[x][y][0];
          PNMPict_G(image, x, y) = answer[x][y][1];
          PNMPict_B(image, x, y) = answer[x][y][2];
        }
      }
      ...
      for (y = 0; y < TESTHEIGHT; y++) {
        for (x = 0; x < TESTWIDTH; x++) {
          if ((PNMPict_R(image, x, y) != answer[x][y][0])
              || (PNMPict_G(image, x, y) != answer[x][y][1])
              || (PNMPict_B(image, x, y) != answer[x][y][2])) {
            is_ok = 0;
            goto CHECK_END_P3P6;
...
/* 実画像の読み書きテスト */
void testPNMBitWriter_ReadWritePNMPictureTest(void *obj)
{
  TEST_UNUSED_PARAMETER(obj);

  {
    struct PNMPicture*  image;
    struct PNMPicture*  test;
    uint32_t i_test;
    uint32_t is_ok, x, y;
    const char*       test_filename[] = {
      "./data/JPEG_ascii.pbm",
    ...
    for (i_test = 0; i_test < sizeof(test_filename) / sizeof(test_filename[0]); i_test++) {
      ...
      image = PNM_CreatePictureFromFile(test_filename[i_test]);
      Test_AssertEqual(PNM_WritePictureToFile(temp_filename, image), PNM_APIRESULT_OK);
      /* 書き出したファイルを読む */
      test  = PNM_CreatePictureFromFile(temp_filename);
      ...
      for (y = 0; y < image->header.height; y++) {
        for (x = 0; x < image->header.width; x++) {
          if (memcmp(&(image->picture[y][x]), &(test->picture[y][x]), sizeof(PNMPixel)) != 0) {
            is_ok = 0;
            goto CHECK_END;
          }
        }
...
void testPNM_Setup(void)
{
  ...
  Test_AddTest(suite, testPNM_CreateFreePictureTest);
  ...
  Test_AddTest(suite, testPNMParser_ReadPNMPictureTest);
  Test_AddTest(suite, testPNMBitWriter_WriteBitTest);
  Test_AddTest(suite, testPNMBitWriter_WritePNMPictureTest);
  Test_AddTest(suite, testPNMBitWriter_ReadWritePNMPictureTest);
