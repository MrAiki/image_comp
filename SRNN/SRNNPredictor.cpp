                             ...
                             int num_mid)
{
  ...
  this->goalError      = float(0.0001);
  this->epsilon        = float(0.00001);
  this->learnRate      = float(0.6);
...
float* SRNNPredictor::predict(float* input)
{
...
float SRNNPredictor::learning(void)
{
  ...
  while (1) {
    ...
    */
    ...
    // 一回以上系列を回していて,収束したと判定したら終了フラグを立てる.
    if ((fabsf(squareError - prevError) < epsilon) && (iteration > len_seqence)) {
      end_flag = 1;
    }
